...
const CATEGORIES = [
  "All", "Breakfast", "South Indian", "Tea & Coffee", "Momos", "Maggi",
    ...
    <div className="min-h-screen bg-[#FAF8F5] pb-16">
      ...
      <div className="bg-white border-b border-stone-200/80 pt-8 pb-6 px-4 sm:px-6 lg:px-8">
        <div className="max-w-4xl mx-auto text-center mb-6">
          <h1 className="font-hero text-3xl sm:text-4xl font-extrabold text-[#1B3B2B]">Majedaar Menu</h1>
        </div>
        <div className="max-w-md mx-auto relative">
          <svg className="w-4 h-4 absolute left-3.5 top-3 text-stone-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          />
          {search && (
            <button onClick={() => setSearch("")} className="absolute right-3.5 top-3 text-stone-400 hover:text-stone-600">
      ...
      {/* Sticky Category Bar */}
      <div id="ordering-menu" className="sticky top-[65px] z-30 bg-white border-b border-stone-200/90 py-3 px-4 sm:px-6 shadow-2xs">
            ...
            return (
              <button
                ...
                className={`whitespace-nowrap px-4 py-1.5 text-xs sm:text-sm font-semibold rounded-full transition-all ${
                  isActive ? "bg-[#1B3B2B] text-white shadow-2xs" : "text-stone-600 hover:text-[#1B3B2B] hover:bg-stone-100"
                }`}
              >
                {cat}
              </button>
            ...
            return (
              ...
              >
                <div className="w-20 h-20 sm:w-24 sm:h-24 rounded-xl overflow-hidden flex-none bg-stone-100 border border-stone-100 relative">
                  ...
                  <img src={getImg(item)} alt={`${item.name} at Majedaar Restaurant`} className="w-full h-full object-cover" loading="lazy" />
                ...
                <div className="flex-1 min-w-0 flex flex-col justify-between h-full">
                  <div>
                    <h3 className="font-bold text-xs sm:text-sm text-[#11261B] truncate mb-0.5">{item.name}</h3>
                    <p className="text-stone-400 text-[11px] line-clamp-2 mb-2 leading-tight">{item.description}</p>
                  </div>
                  <div className="flex items-center justify-between mt-auto">
                    <span className="text-xs sm:text-sm font-extrabold text-[#1B3B2B]">{currency}{displayPrice}</span>
                    <div className="flex items-center gap-1.5">
                      {item.types && item.types.length > 1 && (
                        <select
                          value={defaultType}
                          onChange={(e) => setSelectedTypeMap({ ...selectedTypeMap, [item._id]: e.target.value })}
